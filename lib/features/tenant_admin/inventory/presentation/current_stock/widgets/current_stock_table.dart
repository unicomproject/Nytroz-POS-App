...
class CurrentStockTable extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          padding: const EdgeInsets.only(bottom: TenantAdminSpacing.md, right: TenantAdminSpacing.sm),
          child: Text(
            ...
            style: (Theme.of(context).textTheme.labelSmall ?? const TextStyle()).copyWith(color: TenantAdminColors.mutedText),
          ),
        ),
              ...
              if (stockPage.items.isEmpty)
                Padding(
                  ...
                  child: Center(
                    child: Column(
                      children: [
                        const Icon(Icons.inventory_2_outlined, size: 48, color: TenantAdminColors.mutedText),
                        const SizedBox(height: TenantAdminSpacing.md),
                        const Text(
                          'No matching stock found',
                          style: TextStyle(color: TenantAdminColors.mutedText),
                        ),
                        if (search.isNotEmpty || ref.read(currentStockStatusFilterProvider) != null) ...[
                          const SizedBox(height: TenantAdminSpacing.md),
                          TextButton(
                            onPressed: () {
                  ...
                  builder: (context, constraints) {
                    return SingleChildScrollView(
                      ...
                      child: ConstrainedBox(
                        ...
                        child: DataTable(
                          headingTextStyle: (Theme.of(context).textTheme.labelSmall ?? const TextStyle()).copyWith(color: TenantAdminColors.mutedText, fontWeight: FontWeight.w600),
                          dataTextStyle: (Theme.of(context).textTheme.bodySmall ?? const TextStyle()),
                          dividerThickness: 1,
                              ...
                              children: [
                                Container(
                                  ...
                                  child: item.imageUrl == null || item.imageUrl!.isEmpty
                                      ? const Center(
                                          child: Icon(Icons.inventory_2_outlined, size: 18, color: Colors.blue),
                                        )
                                      : null,
                                ),
                                ...
                                Column(
                                  ...
                                  children: [
                                    Text(
                                      ...
                                      style: (Theme.of(context).textTheme.bodySmall ?? const TextStyle()).copyWith(fontWeight: FontWeight.w600),
                                    ),
                        ...
                        DropdownButtonHideUnderline(
                          child: Container(
                            ...
                            child: DropdownButton<int>(
                              ...
                              onChanged: (newValue) {
                                if (newValue != null) {
                                  ref.read(currentStockPageSizeProvider.notifier).state = newValue;
                      ...
                      children: [
                        OutlinedButton.icon(
                          onPressed: currentPage > 1
                              ? () {
                                  ref.read(currentStockPageProvider.notifier).state = currentPage - 1;
                                }
                              : null,
                          ...
                          style: OutlinedButton.styleFrom(
                            padding: const EdgeInsets.symmetric(horizontal: TenantAdminSpacing.sm),
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(TenantAdminSpacing.sm)),
                            side: BorderSide(color: currentPage > 1 ? TenantAdminColors.border : Colors.transparent),
                          ),
                        ),
                        const SizedBox(width: TenantAdminSpacing.sm),
                        ..._buildPageNumbers(context, ref, currentPage, (stockPage.totalCount / pageSize).ceil()),
                        const SizedBox(width: TenantAdminSpacing.sm),
                        OutlinedButton.icon(
                          onPressed: (currentPage * pageSize) < stockPage.totalCount
                              ? () {
                                  ref.read(currentStockPageProvider.notifier).state = currentPage + 1;
                                }
                              : null,
                          ...
                          style: OutlinedButton.styleFrom(
                            padding: const EdgeInsets.symmetric(horizontal: TenantAdminSpacing.sm),
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(TenantAdminSpacing.sm)),
                            side: BorderSide(color: (currentPage * pageSize) < stockPage.totalCount ? TenantAdminColors.border : Colors.transparent),
                          ),
                        ),
                      ],
                    ),
  ...
  List<Widget> _buildPageNumbers(BuildContext context, WidgetRef ref, int currentPage, int totalPages) {
    ...
    if (totalPages <= 5) {
      ...
      if (currentPage <= 3) {
        ...
        pageIndices.addAll([1, -1, totalPages - 3, totalPages - 2, totalPages - 1, totalPages]);
      } else {
        pageIndices.addAll([1, -1, currentPage - 1, currentPage, currentPage + 1, -1, totalPages]);
      }
    }
      ...
      if (pageNum == -1) {
        pages.add(
          Container(
            ...
            child: const Text('...', style: TextStyle(color: TenantAdminColors.mutedText)),
          ),
        );
      } else {
        ...
        pages.add(
          InkWell(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isActive ? TenantAdminColors.primary : Colors.transparent,
                borderRadius: BorderRadius.circular(TenantAdminSpacing.sm),
                border: Border.all(color: isActive ? TenantAdminColors.primary : Colors.transparent),
              ),
              child: Text(
                pageNum.toString(),
                style: TextStyle(
  ...
  Widget _buildStatusBadge(BuildContext context, String status) {
    ...
    return Container(
      ...
      child: Text(
        label,
        style: (Theme.of(context).textTheme.labelSmall ?? const TextStyle()).copyWith(color: text, fontWeight: FontWeight.w600),
      ),
    );
  }
}
