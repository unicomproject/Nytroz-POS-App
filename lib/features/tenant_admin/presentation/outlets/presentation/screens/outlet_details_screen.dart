...
class OutletDetailsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<OutletDetailsScreen> createState() => _OutletDetailsScreenState();
}
...
class _OutletDetailsScreenState extends ConsumerState<OutletDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final canAddTill = accessState.maybeWhen(
      data: (checker) => checker.canShowAction('tenant_admin.tills', 'tills.create'),
      orElse: () => false,
    );
    final canAddStaff = accessState.maybeWhen(
      data: (checker) => checker.canShowAction('tenant_admin.staff', 'staff.create'),
      orElse: () => false,
    );
        ...
        actions: [
          ...
          if (canUpdate)
            TenantAdminPrimaryButton(
              label: 'Edit outlet',
              icon: Icons.edit,
              onPressed: () => context.go('/tenant-admin/outlets/${outlet.id}/edit'),
            ),
        ],
        child: Column(
          ...
          children: [
            ...
            GridView.count(
              ...
              crossAxisSpacing: 16,
              mainAxisSpacing: 16,
              childAspectRatio: MediaQuery.of(context).size.width < 700 ? 1.08 : 1.55,
              shrinkWrap: true,
                ...
                if (canUpdate)
                  SizedBox(
                    ...
                    child: TenantAdminQuickActionCard(
                      title: 'Edit outlet',
                      icon: Icons.edit,
                      onTap: () => context.go('/tenant-admin/outlets/${outlet.id}/edit'),
                    ),
                  ),
                if (canAddTill)
                  SizedBox(
            ...
            if (outlet.needsAttention.isNotEmpty) ...[
              const SizedBox(height: 24),
              Text('Needs attention', style: TenantAdminTextStyles.sectionTitle(context)),
              const SizedBox(height: 12),
              for (final item in outlet.needsAttention)
                ListTile(
                  leading: const Icon(Icons.warning, color: TenantAdminColors.warning),
