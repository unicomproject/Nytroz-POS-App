...
class OutletImageUploadCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final busy = state.status == OutletImageUploadStatus.uploading ||
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Outlet Image (optional)',
            style: TextStyle(
        ...
        if (state.status == OutletImageUploadStatus.uploading) ...[
          ...
          LinearProgressIndicator(
              value: state.progress, color: TenantAdminColors.posHomeOrangeEnd),
          const SizedBox(height: TenantAdminSpacing.xs),
          Text('Uploading image... ${(state.progress * 100).round()}%',
              style: TenantAdminTextStyles.muted(context)),
        ],
        if (state.errorMessage != null) ...[
          ...
          TextButton(
              onPressed: busy ? null : onRetry, child: const Text('Retry')),
        ],
        const SizedBox(height: TenantAdminSpacing.md),
      ...
      child: Container(
          ...
          decoration: BoxDecoration(
            border: Border.all(
                ...
                style: BorderStyle
                    .solid), // Should ideally be dashed, but standard Border is solid
            borderRadius: BorderRadius.circular(TenantAdminRadius.md),
            color: const Color(0xFFFAFAFA),
          ),
          child: Column(children: [
            const Icon(Icons.cloud_upload_outlined,
                size: 32, color: TenantAdminColors.mutedText),
          ...
          child: AspectRatio(
            ...
            child: hasImage
                ? (state.previewBytes != null
                    ...
                    : Image.network(state.remoteImageUrl!,
                        fit: BoxFit.cover,
