...
class TenantProductRemoteDatasource {
  ...
  Future<ResolveProductBarcodeResponseDto> resolveBarcode(
    ...
    final response = await _dio.post<dynamic>(
      '$_productsPath/barcodes/resolve',
      data: request.toJson(),
    );

    return ResolveProductBarcodeResponseDto.fromJson(
      _unwrapApiPayload(response.data, response.requestOptions),
    );
  }
  ...
  Future<ExternalLookupProductBarcodeResponseDto> externalLookupBarcode({
    ...
    final response = await _dio.post<dynamic>(
      '$_productsPath/barcodes/external-lookup',
      data: {
        'barcode': barcode,
        if (identifierStandard != null && identifierStandard.trim().isNotEmpty)
          'identifierStandard': identifierStandard,
      },
    );

    return ExternalLookupProductBarcodeResponseDto.fromJson(
      _unwrapApiPayload(response.data, response.requestOptions),
    );
  }
